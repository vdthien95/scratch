...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS classes (
    ...
    FOREIGN KEY (teacherEmail) REFERENCES teachers(email) ON DELETE CASCADE
  );
  ...
  CREATE TABLE IF NOT EXISTS students (
    ...
    FOREIGN KEY (classCode) REFERENCES classes(code) ON DELETE CASCADE
  );
`);
...
async function startServer() {
  ...
  app.post("/api/teacher/classes", (req, res) => {
    ...
    try {
      ...
      const teacherExists = db.prepare("SELECT 1 FROM teachers WHERE email = ?").get(teacherEmail);
      if (!teacherExists) {
        return res.status(400).json({ error: "Email giáo viên không tồn tại" });
      }
  ...
  app.delete("/api/teacher/classes/:id", (req, res) => {
    const { id } = req.params;
    try {
      const classInfo = db.prepare("SELECT code FROM classes WHERE id = ?").get(id) as any;
      if (classInfo) {
        // Manually delete students if ON DELETE CASCADE didn't apply to existing tables
        db.prepare("DELETE FROM students WHERE classCode = ?").run(classInfo.code);
  ...
  app.post("/api/auth/student/login", (req, res) => {
    ...
    // Check if class exists first to avoid foreign key constraint error
    const classExists = db.prepare("SELECT 1 FROM classes WHERE code = ?").get(classCode);
    if (!classExists) {
      return res.status(404).json({ error: "Mã lớp không tồn tại" });
    }
